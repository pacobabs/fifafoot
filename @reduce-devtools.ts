...
export const composeWithDevtools = (reducer: (store: any, action: any) => any): ((store: any, action: any) => any) => (
  prevStore,
  action
) => {
  if (action.internal) {
...
const initDevtoolState = (
  ...
  devtoolState = {
    actionsById: {
      0: { action: { type: '@@INIT' }, timestamp: Date.now(), type: 'PERFORM_ACTION' }
    },
    computedStates: [
      {
        state: initialStore
...
const logAction = (action: any, newStore: any) => {
  ...
  devtoolState.actionsById[currentActionId] = { action, timestamp: Date.now(), type: 'PERFORM_ACTION' }
